...
//    private int journalId;
            ...
            try {
                ...
                if (response.isSuccessful() && response.header("Content-Type") != null && Objects.requireNonNull(response.header("Content-Type")).startsWith("application/pdf")) {
                    ...
                    if (uri != null) {
                        ...
                        runOnUiThread(() -> {
                            statusTextView.setText("Загрузка успешно завершена!");
                            viewButton.setEnabled(true);
                            deleteButton.setEnabled(true);
                            addHistoryEntry("Файл " + journalId + ".pdf загружен");
                            openPDFFile(finalUri);
                            journalIdEditText.setText("");
                        });
                    } else {
                        runOnUiThread(() -> statusTextView.setText("Не удалось создать запись в MediaStore."));
                    }
                } else {
                    runOnUiThread(() -> statusTextView.setText("Файл не найден на сервере."));
                }
    ...
    private void showPopupInstruction() {
        ...
        messageView.setText("Добро пожаловать! Для загрузки журнала введите ID и нажмите \"Загрузить\".");
    ...
    private void addHistoryEntry(String message) {
        ...
        String entry = currentDateAndTime + " - " + message;
